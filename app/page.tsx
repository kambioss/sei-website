...
const ui = {
  fr: {
    ...
    newsIntro:
      "Suivez les activités de SEI, ses partenariats et ses contributions à la transformation durable des territoires.",
  ...
  en: {
    ...
    newsIntro:
      "Follow SEI’s activities, partnerships and contributions to the sustainable transformation of territories.",
...
export default async function Home({ searchParams }: HomeProps) {
  ...
  return (
    <main>
      <PageInteractions
        backToTopLabel={locale === "en" ? "Back to top" : "Retour en haut"}
      />
      <PublicHeader
        locale={locale}
      ...
      <section className="hero sectionShell" id="accueil">
        <div className="heroCopy">
          ...
          <div className="heroProof" aria-label={copy.strengths}>
            {hero.proofs.map((proof, index) => (
              <span className="proofItem" key={proof}>
                {proof}
                {index < hero.proofs.length - 1 && <i aria-hidden="true" />}
              </span>
            ))}
          </div>
        </div>
        <div className="heroVisual">
          <Image
            ...
            alt={
              locale === "en"
                ? "African landscape at sunrise"
                : "Paysage africain au lever du soleil"
            }
            fill
            priority
            sizes="(max-width: 110px) 91vw, 42vw"
          />
      ...
      <section className="audienceBand" aria-label={copy.audienceLabel}>
        ...
        <div>
          {audiences.map((audience, index) => (
            <span className="audienceItem" key={audience}>
              {audience}
              {index < audiences.length - 1 && <i aria-hidden="true" />}
            </span>
          ))}
        </div>
      </section>
      ...
      <section className="aboutSection sectionShell" id="sei">
        <div className="aboutTitle">
          ...
          <figure className="aboutTitleMedia">
            <Image
              ...
              alt={
                locale === "en"
                  ? "SEI team on a field visit"
                  : "L’équipe SEI en visite de terrain"
              }
              fill
              unoptimized
        ...
        <div className="aboutCopy">
          {identity.knowUs.paragraphs.map((paragraph, index) => (
            <p
              className={index === 0 ? "aboutLead" : undefined}
              key={paragraph}
            >
              {paragraph}
            </p>
          ...
          <blockquote>
            <span>{copy.ambition}</span>
            {identity.ambition}
          </blockquote>
          <Link className="teaserLink" href={`/qui-sommes-nous?lang=${locale}`}>
        ...
        <section className="newsSection" id="actualites">
          <div className="sectionShell newsSectionInner">
            <div className="sectionIntro">
              <div>
                <p className="eyebrow">
                  <span /> {copy.newsEyebrow}
                </p>
                <h2>{copy.latestNewsTitle}</h2>
              </div>
              <p>{copy.newsIntro}</p>
            </div>
            {articles.length ? (
              <div className="latestNewsBlock">
                  ...
                  {articles.slice(0, 1).map((article) => (
                    <article className="newsCard" key={article.id}>
                      <Link
                        ...
                        href={"/actualites/" + article.slug + "?lang=" + locale}
                      >
                        <Image
                      ...
                      <div className="newsCardCopy">
                        ...
                        <h3>
                          <Link
                            href={
                              "/actualites/" + article.slug + "?lang=" + locale
                            }
                          >
                            {article.title}
                          </Link>
                ...
                <Link
                  className="newsArchiveLink"
                  href={"/actualites?lang=" + locale}
                >
      ...
      <section className="midCta sectionShell">
        <div>
          <p>{valueAdded.label}</p>
          <h2>{valueAdded.text}</h2>
        </div>
        <Link
        ...
        <section className="organizationsSection" id="organisations">
          <div className="sectionShell organizationsInner">
            <div className="organizationsIntro">
              <p className="eyebrow">
                <span /> {trustedOrganizations.eyebrow}
              </p>
            ...
            <OrganizationSlider
              ...
              nextLabel={
                locale === "en" ? "Next organization" : "Organisation suivante"
              }
